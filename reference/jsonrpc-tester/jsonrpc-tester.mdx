...
<Info>
  ...
  Overview](/reference/unified-api/overview-jsonrpc) or browse [chain-specific
  methods](/reference/jsonrpc-docs/1/eth-rpc/eth-get-balance).
</Info>
...
<ParamField query='chainId' type='string' default='1' required>
  Network identifier. Use `1` for Ethereum mainnet. See [supported
  chains](/reference/resources/chains/supported-chains) for all chain IDs.
</ParamField>

<ParamField body='id' type='number' default={1} required>
  Request identifier used to match responses with requests. Can be any number or
  string.
</ParamField>
...
<ParamField body='method' type='string' default='eth_getBalance' required>
  The JSON-RPC method to call. In this example, `eth_getBalance` returns an
  address balance.
</ParamField>

<ParamField body='params' type='array' required>
  Method-specific parameters. For `eth_getBalance`: - `params[0]` - Ethereum
...
<Warning>
  **API key required** - You must add your API key to the headers to make
  successful requests. Get your key from the [Uniblock
...
<Accordion title='Method not found'>
  ...
  [JSON-RPC documentation](/reference/unified-api/overview-jsonrpc) for
  available methods.
</Accordion>
...
<CardGroup cols={2}>
  <Card
    ...
    icon='book'
    href='/reference/unified-api/overview-jsonrpc'
  >
...
<Card
  ...
  icon='link'
  href='/reference/jsonrpc-docs/solana/solana/get-balance'
>
