...
For more information regarding the endpoints, please visit LunarCrush's website: [LunarCrush's Documentation](https://lunarcrush.com/developers/api/endpoints).

# Prefix:

No prefix is required for LunarCrush, an example URL will therefore look like: `https://api.uniblock.dev/direct/v1/LunarCrush` + `/[LunarCrush_endpoint]`.

# Example:

<CodeGroup>
  ```bash curl --request GET \ --url
  https://api.uniblock.dev/direct/v1/LunarCrush/public/nfts/list/v2?sort=percent_change_24h&limit=100&desc=1
