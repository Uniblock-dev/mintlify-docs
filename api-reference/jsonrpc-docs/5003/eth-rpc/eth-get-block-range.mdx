...
<ParamField body='params[0]' type='string' required>
  Start block number (hex) or tag (latest, earliest, pending, safe, finalized).
</ParamField>
<ParamField body='params[1]' type='string' required>
  End block number (hex) or tag (latest, earliest, pending, safe, finalized).
</ParamField>
<ParamField body='params[2]' type='boolean' required>
  Set to true to return full transaction objects, false for only transaction hashes.
</ParamField>
