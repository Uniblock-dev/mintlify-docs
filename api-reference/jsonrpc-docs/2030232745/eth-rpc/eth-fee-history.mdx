...
<ParamField body='params[0]' type='string' required>
  Hex-encoded number of blocks in the requested range (max: 0x400 / 1024 blocks).
</ParamField>
<ParamField body='params[1]' type='string' required>
  Newest block identifier: a hex block number or one of: "latest", "earliest", "pending".
</ParamField>
<ParamField body='params[2]' type='number[]'>
  Optional array of integers representing percentiles (ascending).
</ParamField>
