...
<ParamField body='params[0]' type='object' required>
  Transaction query criteria (TransactionBlockResponseQuery).
</ParamField>
<ParamField body='params[1]' type='string'>
  Optional paging cursor (TransactionDigest). If provided, the query will start from the next item after the cursor.
</ParamField>
<ParamField body='params[2]' type='number' default={100}>
  Maximum number of items returned per page. Defaults to QUERY_MAX_RESULT_LIMIT if not specified.
</ParamField>
<ParamField body='params[3]' type='boolean' default={false}>
  Determines the ordering of results. Default is false (ascending order, oldest record first).
</ParamField>
